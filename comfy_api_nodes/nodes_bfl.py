    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if result["status"] == BFLStatus.ready:
                ...
                # Retry logic for downloading the final image
                download_retries = 5
                for i in range(download_retries):
                    try:
                        img_response = requests.get(img_url, timeout=120) # 120 second timeout
                        img_response.raise_for_status() # Raise an exception for bad status codes
                        return process_image_response(img_response)
                    except requests.exceptions.RequestException as e:
                        if node_id:
                            PromptServer.instance.send_progress_text(
                                f"Download failed ({i+1}/{download_retries}): {e}. Retrying...", node_id
                            )
                        if i < download_retries - 1:
                            time.sleep(5)  # Wait 5 seconds before retrying
                        else:
                            raise Exception(f"Failed to download image after {download_retries} attempts: {e}")
